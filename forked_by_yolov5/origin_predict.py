...
def test(data,
    ...
    for batch_i, (img, targets, paths, shapes) in enumerate(tqdm(dataloader, desc=s)):
        ...
        with torch.no_grad():
            ...
            '''
            targets[:, 2:] = [x, y, w, h] coco coordinate
            '''
            ...
            '''
            pdb.set_trace()
            output_ = output_to_target(output)[:,2:]
            output__ = []
            for a in output_:
                if a[-1] > 0.25:
                    output__.append(a[:4])
    ...
    for i, c in enumerate(ap_class):
        maps[c] = ap[i]
    metric_time = time_synchronized()
    print("load:", load_time-start_time)
    print("data:", data_time-load_time)
    print("forward:", forward_time-data_time)
    print("metric:", metric_time-forward_time)
    print("total:", metric_time-start_time)
